...
func main() {
	...
	http.HandleFunc("/file",redirectFile(1))
	//INICIO DEL SERVIDOR
	serverAddr := "10.0.1.4:5000"
...
func redirectAuth(option int) func(w http.ResponseWriter, r *http.Request) {
    return func(w http.ResponseWriter, r *http.Request) {
        ...
        switch option {
        case 1:
            resp, err = client.Post("https://10.0.2.3:5000" + r.URL.Path, "application/json", r.Body)
...
func redirectFile (option int) func(w http.ResponseWriter, r *http.Request) {
    return func(w http.ResponseWriter, r *http.Request) {
		...
		switch option {
		case 1:
			resp, err = client.Get("https://10.0.2.4:5000" + r.URL.Path)
		case 2:
			resp, err = client.Post("https://10.0.2.4:5000" + r.URL.Path, "application/json", r.Body)
		case 3:
			req, _ := http.NewRequest("PUT", "https://10.0.2.4:5000"+r.URL.Path, r.Body)
			req.Header.Set("Content-Type", "application/json")
			resp, err = client.Do(req)
		case 4:
			req, _ := http.NewRequest("DELETE", "https://10.0.2.4:5000"+r.URL.Path, r.Body)
			req.Header.Set("Content-Type", "application/json")
			resp, err = client.Do(req)
		case 5:
			resp, err = client.Post("https://10.0.2.4:5000" + r.URL.Path, "application/json", r.Body)
...
func RequestToAuthToken(token string, username string) (bool) {
    ...
    req, err := http.NewRequest("POST", "https://10.0.2.3:5000/token", bytes.NewBuffer(requestBody))
    if err != nil {
        return false
    }
